...
if [ "$IS_PROJECT" = false ]; then
    ...
    # 1. Install Binary
    echo -e "📥 ${GREEN}Force updating go-askar CLI to v1.0.3...${NC}"
    GOPROXY=direct go install -v -a github.com/y-as-7/go-askar@v1.0.3 || { echo -e "${RED}❌ Failed to install go-askar. Make sure Go is installed.${NC}"; exit 1; }
