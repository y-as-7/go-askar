...
if [ "$IS_PROJECT" = false ]; then
    ...
    # 1. Install Binary
    echo -e "📥 ${GREEN}Force updating go-askar CLI to v1.3.3...${NC}"
    rm -f $(go env GOPATH)/bin/go-askar || true
    GOPROXY=direct go install -v -a github.com/y-as-7/go-askar/cmd/go-askar@v1.3.3 || { echo -e "${RED}❌ Failed to install go-askar. Make sure Go is installed.${NC}"; exit 1; }
